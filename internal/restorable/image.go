...
func (i *Image) makeStale(rect image.Rectangle) {
	...
	addedRegions := i.appendRegionsForDrawTriangles(nil)
	if !rect.Empty() {
		addedRegions = appendRegionRemovingDuplicates(addedRegions, rect)
	}

	for _, rect := range addedRegions {
		i.staleRegions = appendRegionRemovingDuplicates(i.staleRegions, rect)
	}
...
func (i *Image) appendRegionsForDrawTriangles(regions []image.Rectangle) []image.Rectangle {
	for _, d := range i.drawTrianglesHistory {
		...
		regions = appendRegionRemovingDuplicates(regions, r)
	}
	return regions
}
...
func appendRegionRemovingDuplicates(regions []image.Rectangle, region image.Rectangle) []image.Rectangle {
	...
	return append(regions[:n], region)
}
